...
async def on_ready():
    ...
    activity = discord.Activity(name="@Lilachenn on TikTok", type=discord.ActivityType.watching)
    await bot.change_presence(activity=activity)

    # Display startup information in the console
    display_startup_info(bot.user)
...
if __name__ == '__main__':
    ...
    if system == 'Windows':
        bot.run(token.WINDOWS, log_handler=None)
    else:
        bot.run(token.LINUX, log_handler=None)
